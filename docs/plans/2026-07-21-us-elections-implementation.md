...
## Task 7: Local Section — "Who Represents Alderwood?" (Washington's 21st Legislative District & 1st Congressional District)

**⚠️ CORRECTED mid-implementation — read this before using any fact below.** This section originally targeted WA's 32nd Legislative District and 2nd Congressional District, based on three corroborating scoping-research passes. When Task 7's implementer began Step 1's re-verification, the school-address lookup via `app.leg.wa.gov/DistrictFinder` (flagged even in the original brief as unreachable and only "moderate-high confidence") failed again — but this time the implementer cross-checked directly against the U.S. Census Bureau's own address-level geocoder API instead of stopping at the same dead end, and got a clean, unambiguous match placing Alderwood Middle School (1132 172nd St SW, Lynnwood, WA 98037) in **Legislative District 21** and **Congressional District 1** — not 32/2. The controller independently re-ran the identical Census geocoder query (fresh `curl`, three separate parameter variations, one full all-layers dump to rule out a request mistake) and got the identical result, then separately corroborated it via Wikipedia's "Washington's 1st congressional district" article, which explicitly names Lynnwood and Edmonds in its district description and names **Suzan DelBene (D)** as the current representative. Two independent, authoritative sources agree and neither traces back to the original research pass that produced the LD-32/CD-2 assumption — that original assumption is now confirmed wrong. Likely root cause: Alderwood MS's school district (Edmonds School District) does not follow legislative/congressional boundary lines, and the original research appears to have conflated "which school district" with "which legislative/congressional district."

**What this means for the table below:** every LD-32/CD-2 fact from the original brief (Salomon/Ryu/Davis, Larsen, and all associated 2026 candidate lists) is **WRONG for this specific school** and must not be used. LD-21's actual current legislators and 2026 candidates have NOT yet been researched — this is genuinely new research, not a lookup. CD-1's current representative is confirmed as **Suzan DelBene (D)**, via the corroboration above, but her 2026 candidate/re-election status still needs the same fresh verification every other fact in this task gets. Confirmed via user decision: correct fully to LD-21/CD-1, do not ship any LD-32/CD-2 content, and do not hedge by covering both.
...
**Context:** Approved mid-session as an addition to the original 9-task plan — a locally-relevant section naming the specific officials and 2026 races that directly affect this site's own students (Alderwood Middle School, in the Edmonds School District, WA). This is the highest-nonpartisanship-risk task in the plan alongside Task 6, for a different reason: every one of the confirmed local officeholders happens to currently belong to the same party, which is a plain fact of who represents this specific district, not an editorial choice — the framing discipline below exists specifically to keep it presented that way. (Re-confirm this "same party" premise still holds once LD-21's actual legislators are researched — it was true of the original, incorrect LD-32/CD-2 assumption; verify freshly for LD-21/CD-1 rather than assuming it carries over.)

**Facts still valid from the original scoping research (address-independent — not affected by the district correction):**

| Fact | Confirmed via | Confidence |
|---|---|---|
| Alderwood MS sits in WA's 1st Congressional District (CORRECTED from 2nd) | U.S. Census Bureau address-level geocoder (`geocoding.geo.census.gov`), independently queried twice with matching results; corroborated via Wikipedia's "Washington's 1st congressional district" article naming Lynnwood/Edmonds explicitly | High |
| Alderwood MS sits in WA's 21st Legislative District (CORRECTED from 32nd) | Same Census geocoder result (`2024 State Legislative Districts` layers, both Upper/Senate and Lower/House) | High |
| WA-01 current representative: Suzan DelBene (D) | Wikipedia's "Washington's 1st congressional district" article (direct fetch) | High — re-verify independently at write time per this project's standard discipline; this was a corroborating check during the district correction, not a full Step-1-level re-verification |
| WA's two US Senate seats: Maria Cantwell (term to Jan 2031, next election 2030), Patty Murray (term to Jan 2029, next election 2028) — **neither seat is up in 2026** | senate.gov official roster; independently corroborated by FEC.gov's 2026 WA Senate page showing zero registered candidates | High — address-independent, applies statewide regardless of legislative/congressional district |
| WA's governor is NOT up for election in 2026 (WA governors serve on the presidential cycle: 2024, 2028) | sos.wa.gov, Ballotpedia | High — address-independent, applies statewide |
| 5 of 9 Washington Supreme Court seats (nonpartisan judicial elections, Positions 1, 3, 4, 5, 7) ARE up in 2026 | Ballotpedia "Washington Supreme Court elections, 2026"; TVW's "2026 Video Voters' Guide" (produced with the WA Secretary of State) | High — address-independent, applies statewide; candidate-level detail not researched in depth, verify at write time if this level of detail is included |
| Filing deadline for 2026 WA legislative/congressional races was May 8, 2026; the primary is August 4, 2026 | Local news coverage corroborating filing-week reporting | High — address-independent, applies statewide |
...
| My Edmonds News (myedmondsnews.com) — a local Edmonds-area news outlet. Returns 403 to a raw `curl` check (same bot-blocking pattern already seen on TVW.org/Axios, not itself evidence of being dead) | Task 7's implementer confirmed this same 403-via-curl-but-likely-fine pattern during its own research pass | Not yet independently re-confirmed as live via a non-curl method — treat as unconfirmed until Step 1 checks it |
| KUOW (kuow.org) — Seattle-area NPR member station. Confirmed genuinely live via a direct `curl` check (returns 200, no bot-blocking) | Direct curl check returned 200 | High — genuinely search this source for LD-21/CD-1-relevant coverage at write time; the specific article previously used was about the (now-incorrect) LD-32 race and no longer applies |
| Cascade PBS (cascadepbs.org, articles at cascadepbs.org/articles/) — a Pacific Northwest public-media news outlet. Confirmed genuinely live via direct `curl` checks (both the homepage and `/articles/` return 200) | Confirmed live via direct curl checks only — Task 7's implementer found zero topical fit after three searches for the (now-incorrect) LD-32/CD-2 races; genuinely re-search for LD-21/CD-1 fit before concluding this source has nothing usable, since the earlier null result was for the wrong district | Liveness confirmed; topical fit for the CORRECT district not yet attempted |

**Everything else needs fresh research — this is the actual remaining work for this task:**

- WA-21's current State Senator and two State Representatives (by role — do not assume party composition carries over from the incorrect LD-32 assumption).
- Whether each of WA-21's three legislative seats is up for election in 2026, and if so, every filed candidate by name and party.
- Whether WA-01 (DelBene's seat) is up in 2026 (it is — all U.S. House seats are, every 2 years) and every filed candidate by name and party.
- Candidate platform links (own campaign site or official candidate statement) for every filed candidate in the races above, per the same discipline described below.
- Genuine topical fit (or confirmed lack thereof) for all five named local sources against the CORRECT district's races, not the original incorrect ones.

- [ ] **Step 1: Research every fact above against a live, current, official-record source — this is now genuine research, not verification of pre-supplied facts**

Per this project's established citation discipline: a live/200 URL is necessary but not sufficient — read what it actually says. Prioritize official government sources (leg.wa.gov, senate.gov, congress.gov, sos.wa.gov, FEC.gov) over secondary news coverage wherever a direct fetch is possible; where an official source can't be rendered, corroborate via at least two independent, dated news sources before using a fact. Start from leg.wa.gov's official member roster for WA-21's three legislators (the same primary-source approach the original, now-superseded LD-32 research used successfully), then search HeraldNet/KUOW/Cascade PBS/local sources for 2026 filing news the same way the original research did for LD-32.
...
2. **WA's 21st Legislative District** — name the current State Senator and two State Representatives by role, no party shown (per the confirmed framing decision). For each of the three seats, state plainly whether it's up for election in 2026, and if so, list every filed candidate by name and party (party IS shown here, since a multi-candidate list makes it informational rather than emphasizing one party's dominance — this differs from the officeholder-naming convention above, which deliberately omits party; both conventions were separately confirmed with the project owner, apply each in its correct place). If any seat has a not-seeking-reelection or open-seat situation analogous to the original LD-32 Ryu-runs-for-Senate case, apply the same rule: state the plain fact once, in the entry where it's directly relevant, not repeated across entries.
3. **WA's 1st Congressional District** — same treatment: name the current US Representative (Suzan DelBene, pending final Step 1 re-verification) by role (no party), state the seat is up (all US House seats are, every 2 years), list every filed candidate with party shown.
4. **WA's US Senate seats** — name both current Senators by role (no party), then state plainly and clearly that **neither seat is up for election in 2026** (Senate terms are staggered 6-year terms; briefly note when each seat's next election actually is, tying back to Task 3's generic explanation of staggered Senate terms — Task 3, not Task 5, since the elections-mechanics section moved earlier in the mid-session reorder). This is itself a useful, concrete civics lesson (not every office is on every ballot) — present it that way, not as a gap in coverage. This entry is address-independent and unaffected by the district correction.

**Candidate platform links, added mid-session per explicit project-owner request:** for every filed candidate named in items 2 and 3 above (every race that IS up in 2026 — not the Senate seats, which have no candidates since no race exists), add one link to that candidate's own stated platform in their own words: their official campaign website if one exists, or an official candidate statement (e.g., from the WA Secretary of State's Voters' Guide, which lets every candidate state their own views) if no campaign site can be found. **This project explicitly does NOT characterize, summarize, or describe what any candidate has supported or would support** — that determination was made deliberately after discussing the risk directly with the project owner: describing a candidate's record/positions in this page's own words would require editorial choices about which votes/positions to mention and how to frame them, which is a fundamentally different (and higher) risk than the plain candidate-list facts elsewhere in this section. Only link to the candidate's OWN words — never a third-party article characterizing them. Every filed candidate gets the same treatment, format, and prominence regardless of incumbent/challenger status — if a specific candidate has no findable official website or candidate statement, say so plainly ("no campaign website found") rather than silently omitting that candidate's link or substituting a news article about them.
5. Optionally, if Step 1's research confirms sufficient detail is available: a brief mention that 5 of Washington's 9 Supreme Court seats are also up in 2026 (nonpartisan judicial elections — note explicitly that WA Supreme Court races are nonpartisan by design, which makes this a genuinely lower-risk addition than the partisan races above). Do not force this in if the research doesn't support it cleanly — the 3 partisan-race entries above are the required core of this section.
...
Read the entire section sentence by sentence. Specifically check: does any sentence characterize a race's party composition (e.g., if a race currently has filers from only one party) as good, bad, notable-in-a-charged-way, or a sign of anything? The correct, confirmed framing is to state candidate lists plainly and let the reader observe the composition themselves, exactly the same way a race with candidates from multiple parties would be stated — do not add commentary like "notably" or "interestingly" or "despite" anywhere near a candidate list. If any LD-21 seat has an incumbent-vs-incumbent or not-seeking-reelection situation analogous to the original LD-32 Ryu/Salomon case, apply the same rule confirmed there: state it as plain fact in the entry it's directly relevant to, with any connective "why is this seat open" context living only in that one entry, not repeated or editorialized elsewhere. **Separately, re-confirm the candidate platform links** contain zero characterization anywhere near them — the link label itself should be purely mechanical ("[Name] — campaign website" or "[Name] — no campaign website found"), with no adjective or framing suggesting this page has assessed the quality, credibility, or substance of what any candidate's platform actually says.
...
```bash
git add us-elections.html
git commit -m "feat: add local WA-21/WA-01 representation section to US Elections page"
```
...
Read the ENTIRE page start to finish, specifically hunting for any sentence touching current politics. For each one, apply the test explicitly: would this read the same if the parties currently holding each role were swapped? Pay special attention to: the update-pane's "who controls what" fact block, any mention of specific incumbents who won/lost primaries, the redistricting discussion, and the historical midterm-pattern vocab box (confirm it's framed as historical knowledge, not a 2026 prediction). Give Task 7's local WA-21/WA-01 section this same scrutiny at least as carefully as the national update-pane — specifically re-confirm no candidate list anywhere in that section carries commentary about its partisan composition, and if any seat has an incumbent-vs-incumbent or not-seeking-reelection situation (analogous to the original LD-32 Ryu/Salomon case that this section no longer covers), re-confirm that context appears only once, in the one entry it's directly relevant to, not repeated or editorialized elsewhere. This should be a genuinely separate pass from any fact-checking already done — checking "is this true" and checking "is this neutral" are different questions and both need their own pass.
